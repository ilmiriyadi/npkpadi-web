...
<head>
    ...
    <title>@yield('title', 'Panel Admin - NPK Padi')</title>
...
<body class="text-gray-800 antialiased overflow-x-hidden">
    ...
    <aside id="sidebar" class="bg-white w-72 min-h-screen border-r border-gray-100 fixed top-0 left-0 z-50 transform -translate-x-full md:translate-x-0 transition-transform duration-300 ease-in-out flex flex-col shadow-lg md:shadow-none">
        ...
        <div class="flex items-center justify-between mb-8 px-2">
            <div class="flex items-center">
                <!-- Bulatan Inisial -->
                <div class="w-12 h-12 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center font-bold text-xl border border-blue-200 shadow-sm mr-4">
                ...
                <div>
                    ...
                    <div class="text-xs text-blue-600 font-semibold mt-0.5 flex items-center">
            ...
            <a href="{{ route('admin.settings') }}" class="w-8 h-8 rounded-lg text-gray-400 hover:text-blue-600 hover:bg-blue-50 flex items-center justify-center transition-colors tooltip" title="Edit Profil">
                <i class="fa-solid fa-pen-to-square text-sm"></i>
            </a>
        </div>

        <nav class="p-4 space-y-2 flex-1 overflow-y-auto custom-scrollbar">
            <a href="{{ route('admin.dashboard') }}" class="flex items-center px-4 py-3.5 rounded-xl font-medium transition-colors {{ request()->routeIs('admin.dashboard') ? 'text-green-700 bg-green-50' : 'text-gray-500 hover:text-green-600 hover:bg-green-50' }}">
                <i class="fa-solid fa-house w-6 text-center text-lg mr-3"></i> Dashboard Admin
            </a>
            
            <a href="{{ route('admin.datamaster') }}" class="flex items-center px-4 py-3.5 rounded-xl font-medium transition-colors {{ request()->routeIs('admin.datamaster') ? 'text-green-700 bg-green-50' : 'text-gray-500 hover:text-green-600 hover:bg-green-50' }}">
                <i class="fa-solid fa-database w-6 text-center text-lg mr-3"></i> Data Master
            </a>

            <a href="{{ route('admin.users') }}" class="flex items-center px-4 py-3.5 rounded-xl font-medium transition-colors {{ request()->routeIs('admin.users') ? 'text-green-700 bg-green-50' : 'text-gray-500 hover:text-green-600 hover:bg-green-50' }}">
                <i class="fa-solid fa-users w-6 text-center text-lg mr-3"></i> Manajemen User
            </a>

            <a href="{{ route('admin.history') }}" class="flex items-center px-4 py-3.5 rounded-xl font-medium transition-colors {{ request()->routeIs('admin.history') ? 'text-green-700 bg-green-50' : 'text-gray-500 hover:text-green-600 hover:bg-green-50' }}">
                <i class="fa-solid fa-clock-rotate-left w-6 text-center text-lg mr-3"></i> Pantau Riwayat
            </a>
        </nav>
    ...
    <main class="md:ml-72 pt-16 md:pt-0 min-h-screen flex flex-col transition-all duration-300">
        ...
        <footer class="mt-auto px-6 py-4 border-t border-gray-100 bg-white/50 text-center md:text-left text-sm text-gray-500">
            &copy; 2026 Rice Leaf Nutrition - Politeknik Negeri Banjarmasin. Panel Administrator.
        </footer>
    </main>
